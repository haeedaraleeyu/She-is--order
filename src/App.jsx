...
const GS = {
  app: { minHeight: "100vh", background: "#F0EBE1", fontFamily: "'Palatino Linotype', 'Book Antiqua', Palatino, serif", color: "#1A1A1A" },
  // public
  ...
  stepItem: (active, done) => ({
    ...
    color: done ? "#7A5C2E" : active ? "#F5F0E8" : "#555",
    borderBottom: active ? "2px solid #8B3A2A" : "2px solid transparent",
    transition: "all 0.3s",
  }),
  ...
  colorwayCard: (cw, selected) => ({
    background: selected ? COLORWAY_META[cw].bg : "#FFF",
    border: selected ? `2px solid ${COLORWAY_META[cw].accent}` : "2px solid transparent",
    borderRadius: 8, padding: "14px 16px", marginBottom: 10, cursor: "pointer",
    transition: "all 0.2s", boxShadow: selected ? `0 2px 12px ${COLORWAY_META[cw].accent}44` : "0 1px 4px rgba(0,0,0,0.06)",
  }),
  ...
  sizeBtn: (selected) => ({
    padding: "10px", borderRadius: 6, border: selected ? "2px solid #1A1A1A" : "1px solid #E0DDD8",
    background: selected ? "#1A1A1A" : "#FFF", color: selected ? "#F5F0E8" : "#555",
    fontSize: 13, cursor: "pointer", fontFamily: "inherit", fontStyle: selected ? "italic" : "normal",
  ...
  uploadBox: (hasFile) => ({
    border: hasFile ? "2px solid #7A5C2E" : "2px dashed #C8C0B8",
    borderRadius: 8, padding: "24px 16px", textAlign: "center", cursor: "pointer",
    background: hasFile ? "#EDE0C8" : "#FFF", transition: "all 0.2s",
  }),
  ...
  navRow: { display: "flex", gap: 8, padding: "16px 18px 0" },
  navHalf: (active) => ({ flex: 1, padding: "10px", textAlign: "center", fontSize: 10, letterSpacing: "0.15em", textTransform: "uppercase", cursor: "pointer", borderRadius: 6, background: active ? "#1A1A1A" : "transparent", color: active ? "#F5F0E8" : "#888", border: active ? "none" : "1px solid #E0DDD8", fontFamily: "inherit" }),
  ...
  success: { textAlign: "center", padding: "50px 24px" },
  successIcon: { fontSize: 48, marginBottom: 16 },
  successTitle: { fontSize: 26, fontStyle: "italic", marginBottom: 8, color: "#1A1A1A" },
  successText: { fontSize: 13, color: "#888", lineHeight: 1.7 },
  ...
  adminTitle: { fontSize: 20, fontStyle: "italic", marginBottom: 6 },
  adminSub: { fontSize: 11, color: "#AAA", marginBottom: 24, letterSpacing: "0.05em" },
...
export default function App() {
  ...
  const [step, setStep] = useState(1); // 1 colorway 2 items 3 details 4 payment 5 success
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("she_is_public_orders");
  ...
  const saveOrders = (updated) => {
    setOrders(updated);
    try { localStorage.setItem("she_is_public_orders", JSON.stringify(updated)); } catch {}
  };
    ...
    return (
      <div style={{ ...GS.app, display: "flex", alignItems: "flex-start", justifyContent: "center" }}>
        <div style={GS.adminLogin}>
          <div style={GS.adminTitle}>Admin Access</div>
          <div style={GS.adminSub}>Kreate by ALI · SHE IS</div>
          ...
          />
          <button style={GS.primaryBtn} onClick={() => { if (adminPw === ADMIN_PASSWORD) setAdminAuth(true); else showToast("Incorrect password."); }}>
            Enter
          </button>
      ...
      <div style={GS.app}>
        <div style={{ ...GS.hero, padding: "28px 20px 22px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 12 }}>
            <button style={{ ...GS.navHalf(adminTab === "orders"), flex: "unset", padding: "8px 20px" }} onClick={() => setAdminTab("orders")}>Orders</button>
            <button style={{ ...GS.navHalf(adminTab === "stats"), flex: "unset", padding: "8px 20px" }} onClick={() => setAdminTab("stats")}>Summary</button>
            <button style={{ ...GS.navHalf(false), flex: "unset", padding: "8px 20px", color: "#8B3A2A" }} onClick={() => { setAdminAuth(false); setMode("public"); }}>Exit</button>
          </div>
        </div>
        ...
        {adminTab === "stats" && (
          <>
            ...
            <div style={{ ...GS.totalBox, margin: "12px 18px 0" }}>
              <div style={GS.totalLabel}>Total Revenue (Verified + Fulfilled)</div>
          ...
          <>
            ...
            {orders.length === 0 && (
              <div style={{ textAlign: "center", padding: "50px 20px", color: "#AAA" }}>
                ...
                <div style={{ fontSize: 12 }}>Orders will appear here once customers submit.</div>
              </div>
            )}
            {orders.map(o => (
              <div key={o.id} style={GS.orderCard} onClick={() => setExpandedOrder(expandedOrder === o.id ? null : o.id)}>
                ...
                {expandedOrder === o.id && (
                  <div style={{ marginTop: 12, borderTop: "1px solid #F0EDE8", paddingTop: 12 }}>
                    {o.shirtQty > 0 && <div style={{ fontSize: 12, color: "#888", marginBottom: 2 }}>Shirt × {o.shirtQty} — {formatNaira(o.shirtQty * SHIRT_PRICE)}</div>}
                    {o.trouserQty > 0 && <div style={{ fontSize: 12, color: "#888", marginBottom: 2 }}>Trouser × {o.trouserQty} — {formatNaira(o.trouserQty * TROUSER_PRICE)}</div>}
                    <div style={{ fontSize: 12, color: "#888", marginTop: 6 }}>{o.address}</div>
                    {o.proofPreview && <img src={o.proofPreview} alt="Payment proof" style={GS.proofImg} />}
                    {!o.proofPreview && <div style={{ fontSize: 11, color: "#CCC", marginTop: 8 }}>Payment proof: {o.proofFile}</div>}
  ...
  if (step === 5) {
    return (
      <div style={GS.app}>
        ...
        <div style={GS.success}>
          <div style={GS.successIcon}>✦</div>
          <div style={GS.successTitle}>Order Received.</div>
          <div style={GS.successText}>
            Your payment proof has been submitted.<br />
            We'll verify and confirm your order within 24 hours.<br /><br />
        ...
        <div style={{ textAlign: "center", padding: "0 0 30px" }}>
          <button style={{ background: "none", border: "none", color: "#CCC", fontSize: 11, letterSpacing: "0.1em", cursor: "pointer", fontFamily: "inherit" }} onClick={() => setMode("admin")}>
            Admin
          </button>
        </div>
      </div>
  ...
  return (
    <div style={GS.app}>
      ...
      {step === 2 && (
        <div>
          <div style={GS.section}>
            ...
            {[
              { label: "Shirt", key: "shirtQty", price: SHIRT_PRICE },
              { label: "Trouser", key: "trouserQty", price: TROUSER_PRICE },
            ].map(({ label, key, price }) => (
              <div key={key} style={GS.itemRow}>
                <div>
                  <div style={GS.itemLabel}>{label}</div>
                  <div style={GS.itemPrice}>{formatNaira(price)}</div>
                </div>
          ...
          {(form.shirtQty + form.trouserQty) > 0 && (
            <div style={GS.section}>
              ...
              <div style={GS.sizeGrid}>
                {SIZES.map(s => (
                  <button key={s} style={GS.sizeBtn(form.size === s)} onClick={() => setForm(f => ({ ...f, size: s }))}>{s}</button>
                ))}
              </div>
            </div>
          )}
          ...
          {total > 0 && (
            <div style={GS.totalBox}>
              <div style={GS.totalLabel}>Order Total</div>
        ...
        <div>
          <div style={GS.section}>
            ...
            {[
              ...
              { label: "Instagram Handle", key: "handle", placeholder: "@yourhandle" },
              { label: "Phone Number *", key: "phone", placeholder: "080XXXXXXXX" },
      ...
      {step === 4 && (
        <div>
          <div style={GS.section}>
            <span style={GS.sectionTitle}>Payment</span>
          </div>
          ...
          <div style={GS.totalBox}>
            ...
            <div style={{ fontSize: 11, color: "#666", marginTop: 4 }}>
              {form.shirtQty > 0 && `${form.shirtQty} Shirt${form.shirtQty > 1 ? "s" : ""} · `}
              {form.trouserQty > 0 && `${form.trouserQty} Trouser${form.trouserQty > 1 ? "s" : ""} · `}
              Size {form.size} · {form.colorway}
            </div>
          </div>
          ...
          <div style={GS.section}>
            ...
            <div style={GS.uploadBox(!!form.proofFile)} onClick={() => fileRef.current.click()}>
              {form.proofPreview ? (
                ...
                <>
                  <div style={GS.uploadIcon}>↑</div>
                  <div style={GS.uploadText}>Tap to upload your payment screenshot<br /><span style={{ fontSize: 11, color: "#CCC" }}>JPG, PNG accepted</span></div>
                </>
              )}
            </div>
          ...
          <div style={{ padding: "16px 18px 40px" }}>
            <button style={{ ...GS.primaryBtn, opacity: canNext() ? 1 : 0.4 }} onClick={() => canNext() && submitOrder()}>
              Submit Order
            </button>
      ...
      <div style={{ textAlign: "center", paddingBottom: 20 }}>
        <button style={{ background: "none", border: "none", color: "#DDD", fontSize: 10, letterSpacing: "0.1em", cursor: "pointer", fontFamily: "inherit" }} onClick={() => setMode("admin")}>
          Admin
        </button>
      </div>
